...
class TrainableWavelet(torch.nn.Module):
    def __init__(self, f0=20.0, length=0.096, dt=0.002, device='cuda'):
        super().__init__()
        self.device = device
        self.kernel_size = int(length / dt)

        # Generate the initial Ricker wavelet
        t = torch.arange(-length/2, (length-dt)/2, dt, device=device)
        y = (1 - 2 * (math.pi ** 2) * (f0 ** 2) * (t ** 2)) * torch.exp(-(math.pi ** 2) * (f0 ** 2) * (t ** 2))

        self.wavelet = torch.nn.Parameter(y.clone())  # shape [kernel_size]

    def forward(self, batch_size):
        wavelet = self.wavelet.unsqueeze(0).unsqueeze(1).unsqueeze(3).expand(batch_size, -1, -1, -1)
        wavelet = wavelet / (torch.norm(wavelet, dim=(2,3), keepdim=True) + 1e-6)
        return wavelet  # shape [B, 1, kernel_size, 1]
...
class GaussianDiffusion(nn.Module):
    ...
    def forward_and_backward(self, batch_size=16, img1=None, img2=None, img3=None, t=None, times=None, eval=True):
        """
        Inputs:
            img1: initial impedance image
            img2: seismic data

        Returns:
            Forward: list of images in the forward diffusion process
            Backward: list of images in the reverse sampling process
            final_img: final inversion result image
        """
        self.denoise_fn.eval()
    ...
    def p_losses(self, target, condition, t):
        ...
        # —— 1) Impedance prediction fitting loss ——
        pred_loss = L1MSSSIMLoss()(x_recon, target_norm)

        # —— 2) the wavelet calibration loss ——
